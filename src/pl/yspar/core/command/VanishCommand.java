...
public class VanishCommand extends PlayerCommand
{
    ...
    public static void setVanish(final Player p, final User u) {
        if (u.isVanish() == false) {
                    ...
                    TitleAPI.sendTitle(p, 0, 30, 0, "", Util.fixColor("&6Vanish &aWlaczony"));
                    for (final Player online : Bukkit.getServer().getOnlinePlayers()) {
                        if (online.hasPermission("core.cmd.vanish")) {
                        	ChatUtil.sendMessage(online, ChatUtil.fixColor("&4[!] &cAdministrator &6" + p.getName() + " &aWlaczyl &6vanisha."));
                            continue;
                        }
                        online.hidePlayer(p);
                    }
            ...
            for (final Player online : Bukkit.getServer().getOnlinePlayers()) {
                if (online.hasPermission("core.cmd.vanish")) {
                	ChatUtil.sendMessage(online, ChatUtil.fixColor("&4[!] &cAdministrator &6" + p.getName() + " &cWylaczyl &6vanisha."));
                    continue;
                }
                online.showPlayer(p);
            }
