		...
		while (iterator.hasNext()) {
			...
			if (player.getHealth() >= 4) {
				...
				if (kox > maxKox) {
					...
					player.sendMessage(Util.fixColor("&eNadmiar koxow zostal odlozony do schowka.".replace("{MAX}", Integer.toString(maxKox))
							.replace("{MOVE-SIZE}", Integer.toString(toRemove))));
					ChatUtil.sendActionBar(player, ChatUtil.fixColor("&8&m---&b Nadmiar przedmiotow odlozony do schowka. &8&m---"));
				}
				if (ref > maxRef) {
					...
					player.sendMessage(Util.fixColor("&eNadmiar refili zostal odlozony do schowka.".replace("{MAX}", Integer.toString(maxKox))
							.replace("{MOVE-SIZE}", Integer.toString(toRemove))));
					ChatUtil.sendActionBar(player, ChatUtil.fixColor("&8&m---&b Nadmiar przedmiotow odlozony do schowka. &8&m---"));
				}
				if (lod > 16) {
					...
					player.sendMessage(Util.fixColor("&eNadmiar lodu zostal usuniety".replace("{MAX}", Integer.toString(maxKox))
							.replace("{MOVE-SIZE}", Integer.toString(toRemove))));
				...
				if (perla > maxPerla) {
					...
					player.sendMessage(Util.fixColor("&eNadmiar perel zostal odlozony do schowka.".replace("{MAX}", Integer.toString(maxKox))
							.replace("{MOVE-SIZE}", Integer.toString(toRemove))));
					ChatUtil.sendActionBar(player, ChatUtil.fixColor("&8&m---&b Nadmiar przedmiotow odlozony do schowka. &8&m---"));
				}
				iterator = iterator2;
